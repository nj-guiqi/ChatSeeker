...
# ✅ 断点续存：检查是否已有部分处理数据
if os.path.exists(config["out_put_path"]):
    with open(config["out_put_path"], "r", encoding="utf-8") as f:
        try:
...
def hierarchical_atten_embedding(chunks_list, start_idx=0):
    """从指定索引开始处理，支持断点续存"""
    for i in tqdm(range(start_idx, len(chunks_list)), desc="Processing chunks", unit="chunk"):
        ...
        # 定期保存已处理的数据
        if i % 10 == 0:
            with open(config["out_put_path"], "w", encoding="utf-8") as f:
                result_data = processed_data + chunks_list[last_processed_idx:i + 1]
...
if __name__ == "__main__":
    ...
    hia_data = hierarchical_atten_embedding(chunks_list, start_idx=last_processed_idx)
