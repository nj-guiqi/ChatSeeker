...
class DynamicWindowProcessor:
    ...
    def dynamic_win_llm(self, dialogue_history: List[str], current_query: str, 
        ...
        for _ in range(vote_times):
            ...
            try:
                ...
                predictions.append(tuple(result))  # 使用元组以便于计数
            except Exception as e:
                print(f"解析LLM响应失败: {e}")
                continue
    ...
    def _parse_output(self, response: str) -> List[int]:
        """
        安全解析LLM输出，将各种格式的字符串转换为包含单个整数的列表
        
        Args:
            response: LLM返回的字符串，可能的格式：
                    "[1]", "1", "[1", "1]", "[1,2]", "1,2", "[-1]" 等
        
        Returns:
            包含单个整数的列表，如 [1] 或 [-1]（解析失败时）
        """
...
if __name__ == "__main__":
    config = {
        "data_dir": "./data",
        "output_dir": "./result_without_rerank_without_refine_uncertainty",
