...
def embedding_data(args):
    ...
    for path in os.listdir(data_base_path):
        if path.endswith(".json"):
            ...
            collection_name = f"bge_m3_win_{conv_data_name}_{args.win_size}"
...
def evaulate_qa(args):
    ...
    for i, (eval_data_path, map_data_path) in enumerate(zip(eval_data_list, map_data_list)):
        colleaction_name = f"bge_m3_win_conv_data_{i}_{args.win_size}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--conv_data_path", type=str, default=r"D:\project\myproject\ChatSeeker\data\conv_data", help="chunked data path")
    ...
    for i in range(0, 16):
        ...
        # print(total_recall_ratio[1])
        result_mat.append(total_recall_ratio)
        results.append(np.mean(total_recall_ratio))
    
    # 使用 zip(*matrix) 转置矩阵，然后对每列求最大值
    # column_max = [max(column) for column in zip(*result_mat)]
    column_max = [max(enumerate(col), key=lambda x: x[1]) for col in zip(*result_mat)]
    print(column_max)
        
    # plot results
    plt.plot(range(0, 16), results)
    ...
    with open(r"D:\project\myproject\ChatSeeker\results\recall_ratio.json", "w", encoding="utf-8") as f:
